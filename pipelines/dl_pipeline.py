...
class DlPipeline(L.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        outs = {'y_pred': y_hat.detach(), 'y_true': y.detach(), 'val_loss': loss.detach()}
    ...
    def on_test_epoch_end(self):
        ...
        max_len = max([y['embeddings'].shape[1] for y in self.test_step_outputs])
        embeddings = torch.cat([
            torch.nn.functional.pad(
                x['embeddings'].detach().cpu(),
                (0, 0, 0, max_len - x['embeddings'].shape[1], 0, 0),
            )
            for x in self.test_step_outputs
        ])
        pids = [pid for x in self.test_step_outputs for pid in x['pids']]
