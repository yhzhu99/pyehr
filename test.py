        ...
        for fold in folds:
            ...
            for seed in seeds:
                ...
                if config['task'] == 'outcome':
                    performance_table['accuracy'].append(perf['accuracy'].item())
                    performance_table['auroc'].append(perf['auroc'].item())
                    performance_table['auprc'].append(perf['auprc'].item())
                    ...
                    performance_table['mae'].append(perf['mae'].item())
                    performance_table['mse'].append(perf['mse'].item())
                    performance_table['rmse'].append(perf['rmse'].item())
                    performance_table['r2'].append(perf['r2'].item())
                else:
                    performance_table['accuracy'].append(perf['accuracy'].item())
                    performance_table['auroc'].append(perf['auroc'].item())
                    performance_table['auprc'].append(perf['auprc'].item())
                    ...
                    performance_table['rmse'].append(perf['rmse'].item())
                    performance_table['r2'].append(perf['r2'].item())
    pd.DataFrame(performance_table).to_csv('performance_all_dl.csv', index=False) # [TO-SPECIFY]